...
object SectionTitleParser {
    ...
    private val answerSectionRegex = Regex(
        """(参考答案|标准答案|正确答案|答案(?:与|及)?解析|答案解析|试题答案|答题要点|参考要点|参考思路|答题思路|作答思路|评分要点|答案区|解析区|答案部分)"""
    )
    ...
    private fun isInlineAnswerLine(title: String): Boolean {
        return Regex("""^\s*(?:答案|正确答案|参考答案|标准答案|参考要点|参考思路|答题要点|答题思路|作答思路|评分要点|答)\s*[:：]\s*\S+""").containsMatchIn(title)
    }
