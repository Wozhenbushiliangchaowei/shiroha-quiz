...
object SectionTitleParser {
    ...
    private val sectionLikeRegex = Regex(
        """^\s*(?:第[一二三四五六七八九十百0-9]+(?:部分|卷|章|节|模块)|[一二三四五六七八九十百]+[、.．]\s*(?:常识判断|言语理解|语言理解|数量关系|数学能力|数学运算|判断推理|图形推理|定义判断|类比推理|逻辑判断|资料分析|材料分析|综合知识|公共基础知识|专业知识|基础知识|安全知识|理论知识|综合能力).*)\s*$"""
    )
    private val nonTypeSectionKeywordRegex = Regex("""(部分|试卷|常识判断|言语理解|语言理解|数量关系|数学能力|数学运算|判断推理|图形推理|定义判断|类比推理|逻辑判断|资料分析|材料分析|综合知识|公共基础知识|专业知识|基础知识|安全知识|理论知识|综合能力)""")
